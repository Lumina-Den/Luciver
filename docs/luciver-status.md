...
- **Voice session tracking** – Watches the `Voice Meeting` and `weekly-bash-discussion` voice channels (add more by name or ID via `TRACKED_VOICE_CHANNEL_IDS`) and posts a rich attendance summary in the Luciver log channel once the room clears out. The report covers overtime vs. a configurable base duration, peak/average concurrency, top attendees, first arrivals, and badge annotations (configure via `VOICE_SESSION_BASE_DURATION_MINUTES`, `VOICE_SESSION_MIN_LOG_DURATION_MINUTES`, `VOICE_SESSION_FULL_ATTENDANCE_THRESHOLD`, `VOICE_SESSION_ON_TIME_THRESHOLD`).
