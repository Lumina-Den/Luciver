...
Luciver is the Discord concierge for the Scopio dev server. It reacts to natural language cues that include its name, moves messages between channels, keeps running task and reminder lists, and reports weekly summaries for moderators.
...
| `luciver remind me/<@user> <note + time>` | Luciver remind me to review the deployment plan in 30m | Handles phrases like `in 20m`, `tomorrow 5pm`, `at 4:40am`, or `17 Dec 09:00`, schedules the reminder, confirms the ETA, and logs the entry for audit (defaults to 09:00 if you omit a time). |
